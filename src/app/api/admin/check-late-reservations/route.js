...
export async function POST() {
  try {
    ...
    for (const reservation of todayReservations) {
      ...
      const twentyMinutesLater = new Date(reservationDateTime.getTime() + 20 * 60 * 1000)
      ...
      if (now > twentyMinutesLater) {
        ...
        console.log(`🚨 Réservation ${reservation.id} (${reservation.name}) marquée en retard : ${reservation.time} -> ${now.toLocaleTimeString()}`)
      }
    }
